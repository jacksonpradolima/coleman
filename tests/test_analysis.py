...
def test_run_report_cost_orders_by_higher_apfdc_first(tmp_path):
    _build_parquet_fixture(tmp_path)

    columns, rows = run_report("cost", tmp_path)

    assert columns[:3] == ["policy", "reward_function", "avg_apfdc"]
    assert rows[0][0] == "Random"


def test_run_report_pareto_treats_apfdc_as_maximization(tmp_path):
    root = tmp_path
    root.mkdir(parents=True, exist_ok=True)
    file_path = root / "sample.parquet"
    ...
    con.execute(
        ...
        FROM (
            VALUES
                (
                    's1', 1, 1, 'e1', '1', 'sequential', 'Strong', 'RNFail',
                    'ratio', 0.5,
                    1.0, 0.1, 1.0, 1.0, 10.0, 1.0, 1.0,
                    1, 0, 10, 0, 1.0, 1.0, 0.0, 0.90, 0.90, 0.5, 0.5,
                    'h1', '', NULL
                ),
                (
                    's1', 1, 2, 'e2', '2', 'sequential', 'Weak', 'RNFail',
                    'ratio', 0.5,
                    1.0, 0.2, 2.0, 2.0, 20.0, 2.0, 2.0,
                    1, 0, 10, 0, 1.0, 1.0, 0.0, 0.80, 0.80, 0.7, 0.7,
                    'h2', '', NULL
                )
        ) AS x(
            scenario, experiment, step, execution_id, worker_id, parallel_mode,
